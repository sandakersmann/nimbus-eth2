...
if [[ "${OS}" != "windows" ]]; then
  ...
  if [[ "${RUN_GETH}" == "1" ]]; then
    for GETH_NODE_IDX in $(seq 0 $GETH_LAST_NODE_IDX); do
      for PORT in $(( GETH_NODE_IDX * GETH_PORT_OFFSET + GETH_BASE_NET_PORT )) \
                    $(( GETH_NODE_IDX * GETH_PORT_OFFSET + GETH_BASE_RPC_PORT )) \
                    $(( GETH_NODE_IDX * GETH_PORT_OFFSET + GETH_BASE_WS_PORT )) \
                    $(( GETH_NODE_IDX * GETH_PORT_OFFSET + GETH_BASE_AUTH_RPC_PORT ));
      do
        PORTS_TO_KILL+=("${PORT}")
      done
    done
  ...
  if [[ "${RUN_NIMBUS_ETH1}" == "1" ]]; then
    for NIMBUS_ETH1_NODE_IDX in $(seq 0 $NIMBUS_ETH1_LAST_NODE_IDX); do
      for PORT in $(( NIMBUS_ETH1_NODE_IDX * NIMBUS_ETH1_PORT_OFFSET + 1 + NIMBUS_ETH1_BASE_NET_PORT )) \
                  $(( NIMBUS_ETH1_NODE_IDX * NIMBUS_ETH1_PORT_OFFSET + 1 + NIMBUS_ETH1_BASE_RPC_PORT )) \
                  $(( NIMBUS_ETH1_NODE_IDX * NIMBUS_ETH1_PORT_OFFSET + 1 + NIMBUS_ETH1_BASE_WS_PORT )) \
                  $(( NIMBUS_ETH1_NODE_IDX * NIMBUS_ETH1_PORT_OFFSET + 1 + NIMBUS_ETH1_BASE_AUTH_RPC_PORT ));
      do
        PORTS_TO_KILL+=("${PORT}")
      done
    done
...
EXECUTION_GENESIS_JSON="${DATA_DIR}/local_sim_execution_genesis.json"
sed "s/SHANGHAI_FORK_TIME/${SHANGHAI_FORK_TIME}/g; s/SHARDING_FORK_TIME/${SHARDING_FORK_TIME}/g" \
  "${SCRIPTS_DIR}/local_sim_execution_genesis.json.template" > "$EXECUTION_GENESIS_JSON"
...
if [[ $USE_GANACHE == "0" ]]; then
  ...
  $BEACON_NODE_COMMAND createTestnet \
    ...
    --genesis-time=${GENESIS_TIME} # Delay in seconds
