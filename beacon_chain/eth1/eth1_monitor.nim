...
template makeForkedPayloadAttributes(
    GetPayloadResponseType: type engine_api.ExecutionPayloadV1,
    timestamp: uint64,
    randomData: Eth2Digest,
...
template makeForkedPayloadAttributes(
    GetPayloadResponseType: type engine_api.GetPayloadV2Response,
    timestamp: uint64,
    randomData: Eth2Digest,
    suggestedFeeRecipient: Eth1Address,
    withdrawals: seq[WithdrawalV1]): ForkedPayloadAttributes =
...
func blockValue(blk: ExecutionPayloadV1): int64 {.raises: [RlpError, Defect].} =
  ...
  for transactionBytes in blk.transactions:
    ...
    result += distinctBase effectiveGasTip(transaction, blk.baseFeePerGas)

proc cmpGetPayloadResponses(lhs, rhs: ExecutionPayloadV1): int =
  try:
    cmp(blockValue(lhs), blockValue(rhs))
  except CatchableError as err:
    debug "Failure while decoding transactions", err = err.msg
    return 0

proc cmpGetPayloadResponses(lhs, rhs: GetPayloadV2Response): int =
  # TODO Would this fit in uint64?
  cmp(uint64 lhs.blockValue, uint64 rhs.blockValue)
...
template EngineApiResponseType*(T: type engine_api.ExecutionPayloadV1): type =
  engine_api.ExecutionPayloadV1

template EngineApiResponseType*(T: type engine_api.ExecutionPayloadV2): type =
  engine_api.GetPayloadV2Response

template payload(response: engine_api.ExecutionPayloadV1): engine_api.ExecutionPayloadV1 =
  response

template payload(response: engine_api.GetPayloadV2Response): engine_api.ExecutionPayloadV2 =
...
proc getPayload*(m: ELManager,
                 PayloadType: type engine_api.SomeExecutionPayload,
                 headBlock, safeBlock, finalizedBlock: Eth2Digest,
                 timestamp: uint64,
                 randomData: Eth2Digest,
                 suggestedFeeRecipient: Eth1Address,
  ...
  if bestPayloadIdx.isSome:
    return ok requests[bestPayloadIdx.get].read.payload
  else:
    return err()
...
proc forkchoiceUpdated*(m: ELManager,
  ...
  let
    ...
    requests = m.elConnections.mapIt(
      it.forkchoiceUpdatedForSingleEL(state, payloadAttributes))
    requestsCompleted = allFutures(requests)
...
proc findTerminalBlock(rpcClient: RpcClient,
  ...
  proc next(x: BlockObject): Future[BlockObject] {.async.} =
    ...
    let key = uint64(max(int64(x.number) + step, 0))
...
proc syncEth1Chain(m: ELManager, connection: ELConnection) {.async.} =
  ...
  while true:
    ...
    if shouldCheckForMergeTransition and
       m.currentEpoch >= m.cfg.BELLATRIX_FORK_EPOCH and
       m.terminalBlockHash.isNone and
       latestBlock.totalDifficulty >= m.cfg.TERMINAL_TOTAL_DIFFICULTY:

      info "syncEth1Chain: checking for merge terminal block",
      ...
      debug "syncEth1Chain: found merge terminal block",
        currentEpoch = m.currentEpoch,
        BELLATRIX_FORK_EPOCH = m.cfg.BELLATRIX_FORK_EPOCH,
        ttd = $m.cfg.TERMINAL_TOTAL_DIFFICULTY,
